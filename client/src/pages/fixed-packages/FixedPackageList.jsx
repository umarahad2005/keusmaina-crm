...
const emptyForm = () => ({
    ...
    // Typed by hand — a fixed package's airline and hotels belong to the
    // supplier, so they deliberately do not come from our inventory.
    components: {
        airlineName: '',
        makkahHotel: { hotelName: '', roomType: '', nights: '' },
        madinahHotel: { hotelName: '', roomType: '', nights: '' },
    },
    notes: '',
});
...
export default function FixedPackageList() {
    ...
    const fetchAll = async ({ silent = false } = {}) => {
        ...
        try {
            const [fp, sup, b2c, b2b] = await Promise.all([
            ...
            if (!silent) toast.error(e.response?.data?.message || 'Failed to load fixed packages');
        } finally { if (!silent) setLoading(false); }
    };

    useEffect(() => { fetchAll(); }, []);

    // Hold off while a modal is open so the list can't shuffle under a form.
    useAutoRefresh(() => fetchAll({ silent: true }), { enabled: !modal && !sellItem });
    ...
    const openEdit = (it) => {
        ...
        setForm({
            ...
            components: {
                airlineName: it.components?.airlineName || '',
                makkahHotel: {
                    hotelName: it.components?.makkahHotel?.hotelName || '',
                    roomType: it.components?.makkahHotel?.roomType || '', nights: it.components?.makkahHotel?.nights ?? '',
                },
                madinahHotel: {
                    hotelName: it.components?.madinahHotel?.hotelName || '',
    ...
    const buildPayload = () => {
        const p = {
            ...
            components: {
                airlineName: form.components.airlineName.trim(),
                makkahHotel: { hotelName: form.components.makkahHotel.hotelName.trim(), roomType: form.components.makkahHotel.roomType, nights: num(form.components.makkahHotel.nights) },
                madinahHotel: { hotelName: form.components.madinahHotel.hotelName.trim(), roomType: form.components.madinahHotel.roomType, nights: num(form.components.madinahHotel.nights) },
            },
            notes: form.notes,
        };
        return p;
            ...
            <FormModal isOpen={modal} onClose={() => setModal(false)} title={editId ? 'Edit Fixed Package' : 'Add Fixed Package'} onSubmit={handleSubmit} loading={saving}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div><label className="label">Airline</label>
                        <input className="input" value={form.components.airlineName} onChange={e => setCompField('airlineName', e.target.value)} placeholder="e.g. Saudia SV701" /></div>

                    <div className="sm:col-span-2">
                        <p className="text-[11px] text-gray-500 mb-2">
                            Airline and hotels are typed in, not picked from inventory — the supplier owns these seats and rooms, so they stay out of your own stock.
                        </p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div className="p-2 bg-gray-50 rounded-lg border">
                                <p className="text-xs font-semibold text-navy-800 mb-1">Makkah Hotel</p>
                                <input className="input text-sm mb-1" placeholder="Hotel name" value={form.components.makkahHotel.hotelName} onChange={e => setHotel('makkahHotel', 'hotelName', e.target.value)} />
                            ...
                            <div className="p-2 bg-gray-50 rounded-lg border">
                                ...
                                <input className="input text-sm mb-1" placeholder="Hotel name" value={form.components.madinahHotel.hotelName} onChange={e => setHotel('madinahHotel', 'hotelName', e.target.value)} />
                                <div className="grid grid-cols-2 gap-1">
