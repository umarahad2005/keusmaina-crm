...
export default function HotelsMadinah() {
    ...
    const fetchData = async () => {
        try { setLoading(true); const res = await api.get('/hotels-madinah'); setData(res.data.data); }
    ...
    const columns = [
        ...
        { key: 'distanceFromMasjidNabawi', label: 'Distance' },
    ...
    const handleDelete = async (row) => {
        ...
        try { await api.delete(`/hotels-madinah/${row._id}`); toast.success('Hotel deactivated'); fetchData(); }
    ...
    const handleSubmit = async () => {
        ...
        setSaving(true);
        try {
            if (editId) { await api.put(`/hotels-madinah/${editId}`, form); toast.success('Updated'); }
            else { await api.post('/hotels-madinah', form); toast.success('Created'); }
    ...
    return (
        <div>
            <DataTable columns={columns} data={data} loading={loading}
                ...
                title="Hotels — Madinah" addLabel="Add Hotel" />

            <FormModal isOpen={modal} onClose={() => setModal(false)}
                title={editId ? 'Edit Hotel' : 'Add Hotel (Madinah)'}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div><label className="label">Distance from Masjid Nabawi</label>
